...
Laeme andmed sisse (andmed on siin: https://github.com/RRisto/RRisto.github.io/blob/master/r/data/introductory_state_example.csv):
```{r}
...
```
Eelnevalt me kasutasime pipe oprtaatorit (%>%). See on väga mugav vahend, mitme operatsiooni üksteise järgi tegemiseks. Me ei pea tegema iga vahetulemuse jaoks vahemuutujaid. Mida rohkem muutujaid meil tekib, seda ebaselgemaks keskkond muutub. Näiteks võime kogemata mõne muutuja kuskil mujal üle kirjutada (ja katsu sa siis lõpus aru saada, kus midagi valeks läks). Pipe on väga mugav ja kasulik vahend.
...
Lisaks on paar lihtsat viisi andmete ühendamiseks: __rbind ja cbind__. Esimene eeldab, et kaks andmeraami on samasuguse ridade jaotusega.
```{r}
df1=data.frame(
   emp_id = c (1:5), 
